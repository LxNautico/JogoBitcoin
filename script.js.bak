...
const RankingSystem = {
    ...
    saveGameScore: function(isVictory) {
        ...
        if (playerName !== null) {
            ...
            // Após registrar nome, solicitar avaliação e depois compartilhar
            setTimeout(() => {
                try {
                    abrirAvaliacao(() => abrirCompartilharFinal(nomeLimpo || 'Jogador'));
                } catch (e) {
...
function showHelp() {
    alert(`🎮 AJUDA DO JOGOBITCOIN\n\n1. Escolha dificuldade\n\n2. Descriptografe usando duas ferramentas: Circulo de césar ou Decode,(cada uma delas com sua experiência)\n\n3. Digite resposta\n\n4. Enter\n\n5. Aproveite cada Capítulo da Historia do Bitcoin\n  como pausa do jogo\n\nTipos: Cifra de César, ASCII, Base64`);
}
...
const SkinStore = {
    skins: [
        { id: 'neon', nome: 'Neon Grid', precoBtc: 0.02000000, desc: 'Brilho ciano e magenta, visual futurista.' },
        ...
        { id: 'matrix', nome: 'Matrix Code', precoBtc: 0.02200000, desc: 'Verde terminal com queda de código.' },
        { id: 'lava', nome: 'Lava Core', precoBtc: 0.02100000, desc: 'Vermelho/âmbar incandescente.' },
        { id: 'ice', nome: 'Ice Shards', precoBtc: 0.01900000, desc: 'Azul gelo com brilho cristal.' }
    ],
    
    load() {
        try {
            const o = JSON.parse(localStorage.getItem('jb_skins_owned') || '[]');
            skinsOwned = new Set(o);
        } catch { skinsOwned = new Set(); }
        skinAtiva = localStorage.getItem('jb_skin_active');
        if (skinAtiva) this.apply(skinAtiva, false);
    },
    
    save() {
        localStorage.setItem('jb_skins_owned', JSON.stringify(Array.from(skinsOwned)));
        if (skinAtiva) localStorage.setItem('jb_skin_active', skinAtiva);
    },
    ...
    previewColor(id) {
        ...
        if (id === 'midnight') return 'linear-gradient(135deg, #0b1b2b, #041017)';
        if (id === 'sunset') return 'linear-gradient(135deg, #f46b45, #eea849)';
        return '#888';
    },
...
function saveChatMessages(msgs) {
    localStorage.setItem(CHAT_STORAGE_KEY, JSON.stringify(msgs.slice(-100))); // limita a 100
}

function ensureChatUsername() {
    if (chatUsername) return chatUsername;
    const saved = localStorage.getItem('jb_chat_username');
    if (saved) {
        chatUsername = saved;
        return chatUsername;
    }
    ...
    localStorage.setItem('jb_chat_username', chatUsername);
    return chatUsername;
}
...
function initGameChat() {
    ...
    const quickReply = (text) => {
        ...
        if (lower.includes('conquista') || lower.includes('meta') || lower.includes('selo')) {
            return `Conquistas: velocidade, speedrun, perfeição e 📚 Mente Enciclopédica. Visite todos os verbetes, quebre blocos rápido e mantenha 3 vidas para selos especiais.`;
        }
        // fallback contextual com dica sorteada
        return 'Precisa de ajuda? Use termos: regras, passo a passo, tempo, vidas, hash, chave/cesar, ASCII, Base64, enciclopédia, dificuldade, erro, bloco, som, ranking/score, capítulos, conquistas. Prefixe com "?" para falar direto comigo.';
    };
    ...
    window.addEventListener('storage', (e) => {
        if (e.key === CHAT_STORAGE_KEY) renderMessages();
    });
}
